import { C as VirtualMediaDataSchema, D as createOutputItemSchema, O as createVirtualMedia, a as TOKENS, c as logger, j as generateId, v as ModerationError, y as MultiOutputGenericSchema } from "./dist-Cb5n7sba.mjs";
import { n as createFileAsset } from "./server-m46KSUUy.mjs";
import "./src-goQ_UXNy.mjs";
import { a as defineMetadata, i as defineNode } from "./server-CvuBrF2w.mjs";
import { z as z$1 } from "zod";
import { inject, injectable } from "inversify";
import assert from "node:assert";

//#region ../../nodes/node-video-gen/dist/metadata-B6cJhRfW.mjs
...
const SeedanceConfigSchema = z$1.object({
	...
	seedanceSeed: z$1.number().int().max(2147483646).min(0).optional()
});
const Seedance25ConfigSchema = z$1.object({
	model: z$1.literal("bytedance/seedance-2.5/reference-to-video"),
	seedanceAspectRatio: z$1.enum(SEEDANCE_ASPECT_RATIOS).default("auto"),
	seedanceResolution: z$1.enum(SEEDANCE_25_RESOLUTIONS).default("720p"),
	seedanceDurationSeconds: z$1.enum(SEEDANCE_25_DURATIONS).default("10"),
	seedanceGenerateAudio: z$1.boolean().default(true),
	seedanceSeed: z$1.number().int().max(2147483646).min(0).optional()
});
const WanConfigSchema = z$1.object({
	...
	wanSeed: z$1.number().int().max(2147483647).min(0).optional(),
...
const metadata = defineMetadata({
	...
	pricing: (config, inputs) => {
		...
		if (model.startsWith("bytedance/seedance")) {
			...
			const modelRate = model.startsWith("bytedance/seedance-2.5/") ? .0214 : isFast ? .0112 : .014;
			...
			if ("seedanceDurationSeconds" in config) {
				const durationVal = config.seedanceDurationSeconds;
...
let VideoGenProcessor = class VideoGenProcessor$1 {
	...
	*/
	...
	async processFal({ node, data, resolver, userPrompt, config }) {
		...
		if (config.model === "bytedance/seedance-2.0/reference-to-video" || config.model === "bytedance/seedance-2.0/fast/reference-to-video" || config.model === "bytedance/seedance-2.5/reference-to-video") {
			...
			if (config.model === "bytedance/seedance-2.5/reference-to-video") if (imageUrls.length > 0 || videoUrls.length > 0 || audioUrls.length > 0) {
			...
			} else modelId = "bytedance/seedance-2.5/text-to-video";
			else {
				if (imageUrls.length > 0) input.image_urls = imageUrls;
				if (videoUrls.length > 0) input.video_urls = videoUrls;
				if (audioUrls.length > 0) input.audio_urls = audioUrls;
