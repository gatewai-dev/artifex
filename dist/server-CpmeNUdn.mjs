import { M as generateId, O as createOutputItemSchema, _ as FileDataSchema, a as TOKENS, b as MultiOutputGenericSchema, c as logger, k as createVirtualMedia, y as ModerationError } from "./dist-CQCeP0r0.mjs";
import { t as DataType } from "./dist-BtS_watq.mjs";
import { o as createFileAsset } from "./server-BxjNkdYN.mjs";
...
//#region ../../nodes/node-video-gen-first-last-frame/dist/metadata-BNHW0AUD.mjs
const VIDEOGEN_FAL_MODELS = ["bytedance/seedance-2.0/image-to-video"];
...
const VideoGenFirstLastFrameNodeConfigSchema = z$1.discriminatedUnion("model", [SeedanceImageToVideoConfigSchema, MinimaxImageToVideoConfigSchema]);
const metadata = defineMetadata({
	...
	pricing: (config, inputs) => {
		if (config.model === "bytedance/seedance-2.0/image-to-video") {
			const modelRate = config.model.includes("/fast/") ? .0112 : .014;
			const resolution = config.falResolution || "720p";
			const aspectRatio = config.falAspectRatio || "auto";
			...
			outputDuration = durationVal === "auto" ? 10 : Number(durationVal);
	...
	async #generateWithFal(config, userPrompt, firstUrl, lastUrl, userId) {
		...
		if (config.model === "bytedance/seedance-2.0/image-to-video") {
			input = {
				...
				end_user_id: userId,
				duration: config.falDurationSeconds === "auto" ? "auto" : Number(config.falDurationSeconds),
				aspect_ratio: config.falAspectRatio,
				generate_audio: config.falGenerateAudio,
				bitrate_mode: config.falBitrateMode,
				seed: config.falSeed
			};
			durationSec = config.falDurationSeconds === "auto" ? 10 : Number(config.falDurationSeconds);
