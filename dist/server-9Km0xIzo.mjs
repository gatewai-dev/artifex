import { C as SingleOutputGenericSchema, E as appendOperation, F as getMediaType, N as getActiveMediaMetadata, O as createOutputItemSchema, a as TOKENS, v as MediaMetadataSchema, w as VirtualMediaDataSchema } from "./dist-xnVPaj2K.mjs";
import { a as defineMetadata, i as defineNode } from "./server-Bgx3WrSt.mjs";
import { z as z$1 } from "zod";
import { inject, injectable } from "inversify";

//#region ../../nodes/node-kenburns/dist/metadata-Cp38hW9y.mjs
...
let KenBurnsProcessor = class KenBurnsProcessor$1 {
	...
	async process({ node, data }) {
		try {
			...
			const videoInput = resolver.input().as("Video").item()?.data;
			const imageInput = resolver.input().as("Image").item()?.data;
			const svgInput = resolver.input().as("SVG").item()?.data;
			const inputMedia = videoInput ?? imageInput ?? svgInput;
