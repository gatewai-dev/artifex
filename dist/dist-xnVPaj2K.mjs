...
function isMultiModal(modelName) {
	return AGENT_MODELS.find((m) => m.value === modelName)?.isMultiModal ?? true;
}
...
const bulkUpdateSchema = z$1.object({
	...
	nodes.forEach((node, nodeIndex) => {
		...
		if (node.type === "Compositor") {
			...
			for (const [index, layerConfig] of layerUpdates.entries()) {
...
const VideoSpatialPropsSchema = z$1.object({
	...
	scale: z$1.number().default(1),
	opacity: z$1.number().default(1),
	zIndex: z$1.number().optional(),
	blendMode: GlobalCompositeOperation.optional()
});
const VideoTimingPropsSchema = z$1.object({
	startFrame: z$1.number().default(0),
	durationInMS: z$1.number().optional(),
	trimStart: z$1.number().min(0).optional(),
	trimEnd: z$1.number().min(0).optional(),
...
//#endregion
export { extractSvgDimensions as A, SingleOutputGenericSchema as C, checkNodeResultCache as D, appendOperation as E, getMediaType as F, hasOnlySingleSource as I, isMultiModal as L, generateId as M, getActiveMediaMetadata as N, createOutputItemSchema as O, getFingerprint as P, resolveMediaMimeType as R, NodeResultSchema as S, agentBulkUpdateSchema as T, FileDataSchema as _, TOKENS as a, MultiOutputGenericSchema as b, logger as c, rendererLogger as d, AnyOutputUnionSchema as f, ExportResultSchema as g, DataTypeVal as h, R2StorageService as i, findSourceAsset as j, createVirtualMedia as k, loggerContext as l, DEFAULT_DURATION_MS as m, ENV_CONFIG as n, container as o, BaseVideoLayerPropsSchema as p, GetAssetEndpointBackend as r, getAssetKey as s, APP_VERSION as t, mediaLogger as u, MediaMetadataSchema as v, VirtualMediaDataSchema as w, NodeMetadataSchema as x, ModerationError as y, stampResult as z };