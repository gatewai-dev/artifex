...
import { a as ImageResultSchema, l as VideoResultSchema, u as configBuilder } from "./dist-DgX-LFoS.mjs";
import { z as z$1 } from "zod";
import { inject, injectable } from "inversify";

//#region ../../nodes/node-modulate/dist/metadata-D39QY7gR.mjs
const modulateConfig = configBuilder().field("hue", z$1.number().min(0).max(360).default(0), {
	bindable: true,
	dataTypes: ["Number", "Signal"],
	label: "Hue Signal"
}).field("brightness", z$1.number().min(0).max(2).default(1), {
	bindable: true,
	dataTypes: ["Number", "Signal"],
	label: "Brightness Signal"
}).field("contrast", z$1.number().min(0).max(2).default(1), {
	bindable: true,
	dataTypes: ["Number", "Signal"],
	label: "Contrast Signal"
}).field("exposure", z$1.number().min(-2).max(2).default(0), {
	bindable: true,
	dataTypes: ["Number", "Signal"],
	label: "Exposure Signal"
}).field("saturation", z$1.number().min(0).max(2).default(1), {
	bindable: true,
	dataTypes: ["Number", "Signal"],
	label: "Saturation Signal"
}).field("sepia", z$1.number().min(0).max(1).default(0), {
	bindable: true,
	dataTypes: ["Number", "Signal"],
	label: "Sepia Signal"
}).build();
