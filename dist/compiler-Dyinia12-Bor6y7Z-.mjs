...
import { i as resolveLayerDuration } from "./dist-w8XkXDOQ.mjs";
import { n as ColorSchema } from "./dist-9Gzt6jDx.mjs";
import { z as z$1 } from "zod";

//#region ../../node_modules/.pnpm/yoga-layout@3.2.1/node_modules/yoga-layout/dist/binaries/yoga-wasm-base64-esm.js
var loadYoga$1 = (() => {
	var _scriptDir = import.meta.url;
	return (function(loadYoga$2) {
		loadYoga$2 = loadYoga$2 || {};
		var h;
		h || (h = typeof loadYoga$2 !== "undefined" ? loadYoga$2 : {});
		var aa, ca;
		...
		Mb();
		return loadYoga$2.ready;
	});
})();
var yoga_wasm_base64_esm_default = loadYoga$1;
...
function mapJustify(yoga, value) {
	switch (value) {
...
function mapAlignSelf(yoga, value) {
	switch (value) {
...
function dirOf(node) {
	switch (node.kind) {
		case "flex": return node.dir ?? "row";
		case "block": return "column";
		case "box": return node.children && node.children.length > 0 ? "column" : null;
		default: return null;
	}
}
/** Container default width spec (block fills like HTML). */
function defaultWidthSpec(node) {
	if (node.kind === "block") return "fill";
}
function containerStyle(node) {
	let gap = 0;
	if (node.kind === "flex" || node.kind === "block") gap = typeof node.gap === "number" ? node.gap : 0;
	let padding = 0;
	if (node.kind === "flex" || node.kind === "block" || node.kind === "box" || node.kind === "text") padding = typeof node.padding === "number" ? node.padding : 0;
	let justify = "start";
	if (node.kind === "flex") justify = node.justify ?? "start";
	let align = "start";
	if (node.kind === "flex" || node.kind === "block") align = node.align ?? "start";
	let wrap$1 = false;
	if (node.kind === "flex") wrap$1 = typeof node.wrap === "boolean" ? node.wrap : false;
	return {
		gap,
		padding,
		justify,
...
function flowChildren(node) {
	if ("children" in node && node.children) return node.children.filter((c) => c.position !== "absolute");
	return [];
}
function absoluteChildren(node) {
	if ("children" in node && node.children) return node.children.filter((c) => c.position === "absolute");
	return [];
}
function buildYogaNode(yoga, node, parentExtent, measure, parentDir) {
	...
	if (node.position === "absolute") {
		...
		if (node.x !== void 0 && node.x !== 0) yn.setPosition(yoga.EDGE_LEFT, node.x);
		if (node.y !== void 0 && node.y !== 0) yn.setPosition(yoga.EDGE_TOP, node.y);
	} else yn.setPositionType(yoga.POSITION_TYPE_RELATIVE);
	if (node.grow !== void 0) yn.setFlexGrow(node.grow);
	if (node.flexShrink !== void 0) yn.setFlexShrink(node.flexShrink);
	else yn.setFlexShrink(0);
	if (node.flexBasis !== void 0) yn.setFlexBasis(node.flexBasis);
	if (node.alignSelf !== void 0) yn.setAlignSelf(mapAlignSelf(yoga, node.alignSelf));
	...
	if (isContainer) {
		...
		yn.setFlexDirection(dir === "row" ? yoga.FLEX_DIRECTION_ROW : yoga.FLEX_DIRECTION_COLUMN);
		...
		for (let i = 0; i < allChildren.length; i++) {
			const childMapping = buildYogaNode(yoga, allChildren[i], void 0, measure, node.position === "absolute" ? null : dir);
			yn.insertChild(childMapping.yogaNode, i);
			children.push(childMapping);
		}
	} else if (node.kind === "text" || node.kind === "media" && node.dataType === "Caption") yn.setMeasureFunc((width, widthMode, height, heightMode) => {
		let constraintWidth;
		if (widthMode === 1 || widthMode === 2) constraintWidth = width;
		...
		if (measured) {
			const hasRatio = node.aspectRatio !== void 0;
			const isCaption = node.dataType === "Caption";
			const intrinsicRatio = measured.height > 0 ? measured.width / measured.height : 1;
			if (node.kind === "media" && !isCaption && !hasRatio) yn.setAspectRatio(intrinsicRatio);
			const isWAuto = wSpec === void 0 || wSpec === "auto" || wSpec === "fit";
			const isHAuto = hSpec === void 0 || hSpec === "auto" || hSpec === "fit";
			if (isWAuto && isHAuto) {
				yn.setWidth(measured.width);
				yn.setHeight(measured.height);
			} else if (isWAuto) {
				const ar = node.aspectRatio ?? (node.kind === "media" ? intrinsicRatio : void 0);
				if (ar === void 0 || ar <= 0) yn.setWidth(measured.width);
			} else if (isHAuto) {
				const ar = node.aspectRatio ?? (node.kind === "media" ? intrinsicRatio : void 0);
				if (ar === void 0 || ar <= 0) yn.setHeight(measured.height);
			}
		}
	}
	return {
...
function applySizeSpec(yn, dim, spec, parentExtent, isFlexChild) {
	if (spec === void 0 || spec === "auto" || spec === "fit") return;
	if (typeof spec === "number") {
		if (dim === "width") yn.setWidth(spec);
		else yn.setHeight(spec);
		return;
	}
	if (spec === "fill") {
		if (isFlexChild) {
			if (yn.getFlexGrow() === 0) yn.setFlexGrow(1);
			yn.setFlexBasis(0);
			return;
		}
		if (parentExtent !== void 0) if (dim === "width") yn.setWidth(parentExtent);
		else yn.setHeight(parentExtent);
		else if (dim === "width") yn.setWidthPercent(100);
		else yn.setHeightPercent(100);
	}
}
function readLayout(mapping, parentX, parentY, rects, sizes) {
	...
	rects[mapping.layoutNode.id] = {
		...
		width: Math.round(width * 1e3) / 1e3,
		height: Math.round(height * 1e3) / 1e3
	};
	sizes[mapping.layoutNode.id] = {
		width: Math.round(width * 1e3) / 1e3,
		height: Math.round(height * 1e3) / 1e3
	};
	for (const child of mapping.children) readLayout(child, x, y, rects, sizes);
}
function freeYogaTree(mapping) {
	for (const child of mapping.children) freeYogaTree(child);
	mapping.yogaNode.free();
}
/** Deterministic layout (synchronous): document + viewport (+ measure) → rects. */
function computeLayoutSync(yoga, input) {
	...
	for (const root of layout) {
		...
		const mapping = buildYogaNode(yoga, root, {
		...
		const rootX = root.position === "absolute" ? rootLayout.left : root.x ?? 0;
		const rootY = root.position === "absolute" ? rootLayout.top : root.y ?? 0;
		rects[root.id] = {
			x: Math.round(rootX * 1e3) / 1e3,
			y: Math.round(rootY * 1e3) / 1e3,
			width: Math.round(rootLayout.width * 1e3) / 1e3,
			height: Math.round(rootLayout.height * 1e3) / 1e3
		};
		sizesMap[root.id] = {
			width: Math.round(rootLayout.width * 1e3) / 1e3,
			height: Math.round(rootLayout.height * 1e3) / 1e3
		};
		for (const child of mapping.children) readLayout(child, rootX, rootY, rects, sizesMap);
...
async function computeLayout(input) {
	return computeLayoutSync(await initLayout(), input);
}
/**
...
function collectNodeOps(nodes) {
	const out = [];
	const walk = (vm) => {
		if (vm?.operation?.op === "CompositorLayer") out.push(vm);
...
var _config = {
	...
	var vars = _isObject(v) ? v : { each: v }, ease = _parseEase(vars.ease), from = vars.from || 0, base = parseFloat(vars.base) || 0, cache$1 = {}, isDecimal = from > 0 && from < 1, ratios = isNaN(from) || isDecimal, axis = vars.axis, ratioX = from, ratioY = from;
	...
	return function(i, target, a) {
		var l = (a || vars).length, distances = cache$1[l], originX, originY, x, y, d, j, max, min, wrapAt;
		if (!distances) {
...
var _gsap = {
	...
	quickSetter: function quickSetter(target, property, unit) {
		...
		target = target[0] || {};
		var Plugin = _plugins[property], cache$1 = _getCache(target), p = cache$1.harness && (cache$1.harness.aliases || {})[property] || property, setter = Plugin ? function(value) {
			var p$1 = new Plugin();
			_quickTween._pt = 0;
			p$1.init(target, unit ? value + unit : value, _quickTween, 0, [target]);
			p$1.render(1, p$1);
			_quickTween._pt && _renderPropTweens(1, _quickTween);
		} : cache$1.set(target, p);
		return Plugin ? setter : function(value) {
			return setter(target, p, unit ? value + unit : value, cache$1, 1);
		};
	},
...
var _win, _doc, _docElement, _pluginInitted, _tempDiv, _recentSetterPlugin, _reverting, _windowExists = function _windowExists$2() {
	...
	if (property in _transformProps && style) {
		...
		if (property !== "transform") {
			...
			}) : this.tfm[property] = cache$1.x ? cache$1[property] : _get(target, property);
			property === _transformOriginProp && (this.tfm.zOrigin = cache$1.zOrigin);
	...
	var props = this.props, target = this.target, style = target.style, cache$1 = target._gsap, i, p;
	...
	if (this.tfm) {
		for (p in this.tfm) cache$1[p] = this.tfm[p];
		if (cache$1.svg) {
			cache$1.renderTransform();
			target.setAttribute("data-svg-origin", this.svgo || "");
		}
		i = _reverting();
		if ((!i || !i.isStart) && !style[_transformProp]) {
			_removeIndependentTransforms(style);
			if (cache$1.zOrigin && style[_transformOriginProp]) {
				style[_transformOriginProp] += " " + cache$1.zOrigin + "px";
				cache$1.zOrigin = 0;
				cache$1.renderTransform();
			}
			cache$1.uncache = 1;
	...
	var curValue = parseFloat(value) || 0, curUnit = (value + "").trim().substr((curValue + "").length) || "px", style = _tempDiv.style, horizontal = _horizontalExp.test(property), isRootSVG = target.tagName.toLowerCase() === "svg", measureProperty = (isRootSVG ? "client" : "offset") + (horizontal ? "Width" : "Height"), amount = 100, toPixels = unit === "px", toPercent = unit === "%", px, parent, cache$1, isSVG;
	...
	cache$1 = parent._gsap;
	if (cache$1 && toPercent && cache$1.width && horizontal && cache$1.time === _ticker.time && !cache$1.uncache) return _round(curValue / cache$1.width * amount);
	else {
		...
		if (horizontal && toPercent) {
			cache$1 = _getCache(parent);
			cache$1.time = _ticker.time;
			cache$1.width = parent[measureProperty];
		}
	}
	...
	if (data.tween && data.tween._time === data.tween._dur) {
		var target = data.t, style = target.style, props = data.u, cache$1 = target._gsap, prop, clearTransforms, i;
		...
		if (clearTransforms) {
			...
			if (cache$1) {
				cache$1.svg && target.removeAttribute("transform");
	...
	var cache$1 = target._gsap || _getCache(target), style = target.style, matrix = _getComputedTransformMatrixAsArray(target), parent, nextSibling, temp, addedToDOM;
	if (cache$1.svg && target.getAttribute("transform")) {
	...
	} else if (matrix === _identity2DMatrix && !target.offsetParent && target !== _docElement && !cache$1.svg) {
		temp = style.display;
		style.display = "block";
	...
	var cache$1 = target._gsap, matrix = matrixArray || _getMatrix(target, true), xOriginOld = cache$1.xOrigin || 0, yOriginOld = cache$1.yOrigin || 0, xOffsetOld = cache$1.xOffset || 0, yOffsetOld = cache$1.yOffset || 0, a = matrix[0], b = matrix[1], c = matrix[2], d = matrix[3], tx = matrix[4], ty = matrix[5], originSplit = origin.split(" "), xOrigin = parseFloat(originSplit[0]) || 0, yOrigin = parseFloat(originSplit[1]) || 0, bounds, determinant, x, y;
	if (!originIsAbsolute) {
	...
	if (smooth || smooth !== false && cache$1.smooth) {
		tx = xOrigin - xOriginOld;
		ty = yOrigin - yOriginOld;
		cache$1.xOffset = xOffsetOld + (tx * a + ty * c) - tx;
		cache$1.yOffset = yOffsetOld + (tx * b + ty * d) - ty;
	} else cache$1.xOffset = cache$1.yOffset = 0;
	cache$1.xOrigin = xOrigin;
	cache$1.yOrigin = yOrigin;
	cache$1.smooth = !!smooth;
	cache$1.origin = origin;
	cache$1.originIsAbsolute = !!originIsAbsolute;
	target.style[_transformOriginProp] = "0px 0px";
	if (pluginToAddPropTweensTo) {
		_addNonTweeningPT(pluginToAddPropTweensTo, cache$1, "xOrigin", xOriginOld, xOrigin);
		_addNonTweeningPT(pluginToAddPropTweensTo, cache$1, "yOrigin", yOriginOld, yOrigin);
		_addNonTweeningPT(pluginToAddPropTweensTo, cache$1, "xOffset", xOffsetOld, cache$1.xOffset);
		_addNonTweeningPT(pluginToAddPropTweensTo, cache$1, "yOffset", yOffsetOld, cache$1.yOffset);
	}
	target.setAttribute("data-svg-origin", xOrigin + " " + yOrigin);
}, _parseTransform = function _parseTransform$1(target, uncache) {
	var cache$1 = target._gsap || new GSCache(target);
	if ("x" in cache$1 && !uncache && !cache$1.uncache) return cache$1;
	var style = target.style, invertedScaleX = cache$1.scaleX < 0, px = "px", deg = "deg", cs = getComputedStyle(target), origin = _getComputedProperty(target, _transformOriginProp) || "0", x = y = z$2 = rotation = rotationX = rotationY = skewX = skewY = perspective = 0, y, z$2, scaleX = scaleY = 1, scaleY, rotation, rotationX, rotationY, skewX, skewY, perspective, xOrigin, yOrigin, matrix, angle, cos, sin, a, b, c, d, a12, a22, t1, t2, t3, a13, a23, a33, a42, a43, a32;
	cache$1.svg = !!(target.getCTM && _isSVG(target));
	if (cs.translate) {
		if (cs.translate !== "none" || cs.scale !== "none" || cs.rotate !== "none") style[_transformProp] = (cs.translate !== "none" ? "translate3d(" + (cs.translate + " 0 0").split(" ").slice(0, 3).join(", ") + ") " : "") + (cs.rotate !== "none" ? "rotate(" + cs.rotate + ") " : "") + (cs.scale !== "none" ? "scale(" + cs.scale.split(" ").join(",") + ") " : "") + (cs[_transformProp] !== "none" ? cs[_transformProp] : "");
		style.scale = style.rotate = style.translate = "none";
	}
	matrix = _getMatrix(target, cache$1.svg);
	if (cache$1.svg) {
		if (cache$1.uncache) {
			t2 = target.getBBox();
			origin = cache$1.xOrigin - t2.x + "px " + (cache$1.yOrigin - t2.y) + "px";
			t1 = "";
		} else t1 = !uncache && target.getAttribute("data-svg-origin");
		_applySVGOrigin(target, t1 || origin, !!t1 || cache$1.originIsAbsolute, cache$1.smooth !== false, matrix);
	}
	xOrigin = cache$1.xOrigin || 0;
	yOrigin = cache$1.yOrigin || 0;
	if (matrix !== _identity2DMatrix) {
		...
		if (cache$1.svg) {
			...
			cache$1.forceCSS = target.setAttribute("transform", "") || !_isNullTransform(_getComputedProperty(target, _transformProp));
	...
	uncache = uncache || cache$1.uncache;
	cache$1.x = x - ((cache$1.xPercent = x && (!uncache && cache$1.xPercent || (Math.round(target.offsetWidth / 2) === Math.round(-x) ? -50 : 0))) ? target.offsetWidth * cache$1.xPercent / 100 : 0) + px;
	cache$1.y = y - ((cache$1.yPercent = y && (!uncache && cache$1.yPercent || (Math.round(target.offsetHeight / 2) === Math.round(-y) ? -50 : 0))) ? target.offsetHeight * cache$1.yPercent / 100 : 0) + px;
	cache$1.z = z$2 + px;
	cache$1.scaleX = _round(scaleX);
	cache$1.scaleY = _round(scaleY);
	cache$1.rotation = _round(rotation) + deg;
	cache$1.rotationX = _round(rotationX) + deg;
	cache$1.rotationY = _round(rotationY) + deg;
	cache$1.skewX = skewX + deg;
	cache$1.skewY = skewY + deg;
	cache$1.transformPerspective = perspective + px;
	if (cache$1.zOrigin = parseFloat(origin.split(" ")[2]) || !uncache && cache$1.zOrigin || 0) style[_transformOriginProp] = _firstTwoOnly(origin);
	cache$1.xOffset = cache$1.yOffset = 0;
	cache$1.force3D = _config.force3D;
	cache$1.renderTransform = cache$1.svg ? _renderSVGTransforms : _supports3D ? _renderCSSTransforms : _renderNon3DTransforms;
	cache$1.uncache = 0;
	return cache$1;
...
}, _renderNon3DTransforms = function _renderNon3DTransforms$1(ratio, cache$1) {
	cache$1.z = "0px";
	cache$1.rotationY = cache$1.rotationX = "0deg";
	cache$1.force3D = 0;
	_renderCSSTransforms(ratio, cache$1);
}, _zeroDeg = "0deg", _zeroPx = "0px", _endParenthesis = ") ", _renderCSSTransforms = function _renderCSSTransforms$1(ratio, cache$1) {
	var _ref = cache$1 || this, xPercent = _ref.xPercent, yPercent = _ref.yPercent, x = _ref.x, y = _ref.y, z$2 = _ref.z, rotation = _ref.rotation, rotationY = _ref.rotationY, rotationX = _ref.rotationX, skewX = _ref.skewX, skewY = _ref.skewY, scaleX = _ref.scaleX, scaleY = _ref.scaleY, transformPerspective = _ref.transformPerspective, force3D = _ref.force3D, target = _ref.target, zOrigin = _ref.zOrigin, transforms = "", use3D = force3D === "auto" && ratio && ratio !== 1 || force3D === true;
...
}, _renderSVGTransforms = function _renderSVGTransforms$1(ratio, cache$1) {
	var _ref2 = cache$1 || this, xPercent = _ref2.xPercent, yPercent = _ref2.yPercent, x = _ref2.x, y = _ref2.y, rotation = _ref2.rotation, skewX = _ref2.skewX, skewY = _ref2.skewY, scaleX = _ref2.scaleX, scaleY = _ref2.scaleY, target = _ref2.target, xOrigin = _ref2.xOrigin, yOrigin = _ref2.yOrigin, xOffset = _ref2.xOffset, yOffset = _ref2.yOffset, forceCSS = _ref2.forceCSS, tx = parseFloat(x), ty = parseFloat(y), a11, a21, a12, a22, temp;
...
var CSSPlugin = {
	...
	init: function init(target, vars, tween, index, targets) {
		var props = this._props, style = target.style, startAt = tween.vars.startAt, startValue, endValue, endNum, startNum, type, specialProp, p, startUnit, endUnit, relative, isTransformRelated, transformPropTween, cache$1, smooth, hasPriority, inlineProps, finalTransformValue;
		_pluginInitted || _initCore();
			...
			if (specialProp) specialProp(this, target, p, endValue, tween) && (hasPriority = 1);
				...
				if (isTransformRelated) {
					...
					if (!transformPropTween) {
						cache$1 = target._gsap;
						cache$1.renderTransform && !vars.parseTransform || _parseTransform(target, vars.parseTransform);
						smooth = vars.smoothOrigin !== false && cache$1.smooth;
						transformPropTween = this._pt = new PropTween(this._pt, style, _transformProp, 0, 1, cache$1.renderTransform, cache$1, 0, -1);
						transformPropTween.dep = 1;
					}
					if (p === "scale") {
						this._pt = new PropTween(this._pt, cache$1, "scaleY", cache$1.scaleY, (relative ? _parseRelative(cache$1.scaleY, relative + endNum) : endNum) - cache$1.scaleY || 0, _renderCSSProp);
						this._pt.u = 0;
						props.push("scaleY", p);
						p += "X";
						...
						if (cache$1.svg) _applySVGOrigin(target, endValue, 0, smooth, 0, this);
						else {
							endUnit = parseFloat(endValue.split(" ")[2]) || 0;
							endUnit !== cache$1.zOrigin && _addNonTweeningPT(this, cache$1, "zOrigin", cache$1.zOrigin, endUnit);
						...
						_addRotationalPropTween(this, cache$1, p, startNum, relative ? _parseRelative(startNum, relative + endValue) : endValue);
						continue;
					} else if (p === "smoothOrigin") {
						_addNonTweeningPT(this, cache$1, "smooth", cache$1.smooth, endValue);
						continue;
					} else if (p === "force3D") {
						cache$1[p] = endValue;
						continue;
				...
				if (isTransformRelated || (endNum || endNum === 0) && (startNum || startNum === 0) && !_complexExp.test(endValue) && p in style) {
					...
					this._pt = new PropTween(this._pt, isTransformRelated ? cache$1 : style, p, startNum, (relative ? _parseRelative(startNum, relative + endNum) : endNum) - startNum, !isTransformRelated && (endUnit === "px" || p === "zIndex") && vars.autoRound !== false ? _renderRoundedCSSProp : _renderCSSProp);
...
//#endregion
//#region ../../nodes/node-compositor/dist/compiler-Dyinia12.mjs
...
function getCachedTimeline(cacheKey) {
	...
	if (entry) {
		if (Date.now() - entry.cacheTime > 1e4) {
...
function setCachedTimeline(cacheKey, value) {
	const now = Date.now();
	for (const [key, entry] of cache.entries()) if (now - entry.cacheTime > 1e4) cache.delete(key);
	if (cache.size >= 50) {
...
function compileTrackToTimeline(tl, stub, track, startFrame, fps, durationFrames) {
...
function compileTimeline(renderId, virtualMedia, options) {
	...
	const cacheKey = `${renderId}_${fnv1a(stableStringify({
	...
	nodeOps.forEach((vm, index) => {
		...
		for (const track of lop.animation?.tracks ?? []) if (track.keyframes && track.keyframes.length > 0) {
			const kf0 = track.keyframes[0];
			if (kf0.frame === 0) stub[track.prop] = kf0.value;
		}
	...
	nodeOps.forEach((vm) => {
		...
		for (const track of lop.animation?.tracks ?? []) {
			const { dropped, maxAuthoredFrame } = compileTrackToTimeline(tl, stub, track, startFrame, fps, durationFrames);
...
function compileLayerTimeline(layerId, animation, baseVolume, baseMuted, fps) {
	...
	if (cached) {
		if (Date.now() - cached.cacheTime <= 1e4) {
...
//#endregion
export { compositorToProgram as a, collectNodeOps as i, compileTimeline as n, computeLayout as o, CompositorProgramSchema as r, initLayout as s, compileLayerTimeline as t };