...
//#region ../../nodes/node-liquify/dist/renderer.mjs
...
function getLiquifyResources(device, format) {
	...
	if (!layouts) {
		layouts = {
			...
			singleTextureLayout: device.createBindGroupLayout({
	...
	if (!pipeline) {
		...
		pipeline = device.createRenderPipeline({
			...
			layout: device.createPipelineLayout({ bindGroupLayouts: [layouts.uniformLayout, layouts.singleTextureLayout] }),
			vertex: {
				module,
				entryPoint: "vs"
			},
...
const LiquifyWebGPURenderer = async (args) => {
	...
	const childTexture = ctx.renderer.getTemporaryTexture(width, height, [...props.excludeTextures || [], targetTexture]);
	...
	}, childView, childTexture, width, height);
	...
	for (let i = 0; i < opCount; i++) {
		...
		if (!item) continue;
		const offset = 4 + i * 8;
		uniformData[offset + 0] = TYPE_ENUM_MAP[item.type] ?? 2;
		uniformData[offset + 1] = item.x;
		uniformData[offset + 2] = item.y;
		uniformData[offset + 3] = item.radius ?? .15;
		uniformData[offset + 4] = item.strength ?? .5;
		uniformData[offset + 5] = item.dx ?? 0;
		uniformData[offset + 6] = item.dy ?? 0;
	...
	const textureBindGroup = ctx.renderer.bindGroupCache.getBindGroup(ctx.device, resources.singleTextureLayout, childTexture, sampler);
...
var renderers_default = defineRenderer({ WebGPURenderer: LiquifyWebGPURenderer });
