...
//#region ../../nodes/node-flip/dist/renderer.mjs
...
function getFlipResources(device, targetFormat) {
	...
	const shaderModule = device.createShaderModule({ code: WGSL_FLIP_SHADER });
...
const WebGPURenderer = async (args) => {
	...
	const sourceWidth = childMedia.metadata?.width ?? props.containerWidth ?? width;
	const sourceHeight = childMedia.metadata?.height ?? props.containerHeight ?? height;
	...
	const { pipeline, uniformLayout: uLayout, textureLayout: tLayout } = getFlipResources(ctx.device, ctx.renderer.format);
	const sampler = ctx.renderer.samplerCache.getSampler(ctx.device);
	const mode = op.mode ?? "horizontal";
	let isFlipH = false;
	let isFlipV = false;
	let isDiag = false;
	if (mode === "horizontal") isFlipH = true;
	else if (mode === "vertical") isFlipV = true;
	else if (mode === "both") {
		isFlipH = true;
		isFlipV = true;
	} else if (mode === "diagonal") isDiag = true;
	...
	let symVal = 0;
	if (op.symmetry === "leftToRight") symVal = 1;
	else if (op.symmetry === "rightToLeft") symVal = 2;
	else if (op.symmetry === "topToBottom") symVal = 3;
	else if (op.symmetry === "bottomToTop") symVal = 4;
	else if (op.symmetry === "quadrant") symVal = 5;
	uniformData[0] = isFlipH ? 1 : 0;
	uniformData[1] = isFlipV ? 1 : 0;
	uniformData[2] = isDiag ? 1 : 0;
