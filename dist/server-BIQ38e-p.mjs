...
//#region ../../nodes/node-depth-map/dist/metadata-5SdjpmMZ.mjs
const DepthMapNodeConfigSchema = z$1.object({
	num_inference_steps: z$1.number().int().catch(10),
	ensemble_size: z$1.number().int().catch(10),
	processing_res: z$1.number().int().catch(0)
});
...
let DepthMapProcessor = class DepthMapProcessor$1 {
	...
	async process({ node, data }) {
		try {
			...
			const inputAsImage = this.graph.forNode(node, data).input("Image").asImage();
