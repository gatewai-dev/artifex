import { E as checkNodeResultCache, L as stampResult, M as getActiveMediaMetadata, T as appendOperation, a as TOKENS, c as logger, f as AnyOutputUnionSchema, y as MultiOutputGenericSchema } from "./dist-Cb5n7sba.mjs";
import { a as defineMetadata, i as defineNode } from "./server-CvuBrF2w.mjs";
import { z as z$1 } from "zod";
import { inject, injectable } from "inversify";

//#region ../../nodes/node-smart-cut/dist/metadata-CapDSALV.mjs
const SmartCutNodeConfigSchema = z$1.object({
	paddingLeftMs: z$1.number().int().min(0).max(5e3).default(200).describe("Left padding in milliseconds"),
	paddingRightMs: z$1.number().int().min(0).max(5e3).default(200).describe("Right padding in milliseconds"),
	mergeThresholdMs: z$1.number().int().min(0).max(5e3).default(500).describe("Merge threshold in milliseconds for adjacent speech segments"),
	minSegmentDurationMs: z$1.number().int().min(0).max(2e3).default(100).describe("Minimum duration of a speech segment in milliseconds to keep"),
	chunkLevel: z$1.enum(["word", "segment"]).default("word").describe("Precision level: word or segment"),
	language: z$1.string().optional().describe("VAD speech recognition language hint")
});
