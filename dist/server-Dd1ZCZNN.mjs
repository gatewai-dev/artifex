import { C as getActiveMediaMetadata, i as ExportResultSchema, w as getFingerprint } from "./dist-DSiNOGGx.mjs";
...
//#region ../../nodes/node-export/dist/metadata-DrvHBxn4.mjs
const DEFAULT_RENDER_COST = 20;
const ExportNodeConfigSchema = z$1.object({
	file: z$1.string().min(1).optional(),
	format: z$1.enum([
		"mp4",
		"webm",
		"gif",
...
const metadata = defineMetadata({
	...
	validation: (config, inputs) => {
		if (config?.format === "mp3" || config?.format === "svg") return null;
		...
		if (width != null && height != null) {
			if (width % 2 !== 0 || height % 2 !== 0) errors.dimensions = `Video dimensions must be even numbers. Current size: ${width}x${height}. Please adjust the dimensions to be even numbers (e.g. by cropping or resizing) before exporting.`;
		}
		if (config?.format === "gif") {
			...
			if (durationMs != null && durationMs > 15e3) errors.gifDuration = `GIF export is limited to a maximum duration of 15 seconds. Current duration: ${(durationMs / 1e3).toFixed(1)}s.`;
		}
	...
	async process({ node, data }) {
		try {
			...
			if (config?.file) try {
				...
				const svgContent = dataToPass?.operation?.svgContent ?? inputValue.data?.operation?.svgContent;
				const svgDataUrl = dataToPass?.operation?.svgDataUrl ?? inputValue.data?.operation?.svgDataUrl;
				if (key) buffer = await this.storage.getFromStorage(key);
				else if (typeof svgContent === "string") buffer = Buffer.from(svgContent, "utf-8");
				else if (typeof svgDataUrl === "string" && svgDataUrl.startsWith("data:image/svg+xml;base64,")) buffer = Buffer.from(svgDataUrl.replace("data:image/svg+xml;base64,", ""), "base64");
