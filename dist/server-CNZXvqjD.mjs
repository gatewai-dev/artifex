import { L as isMultiModal, O as createOutputItemSchema, a as TOKENS, b as MultiOutputGenericSchema, c as logger, j as findSourceAsset, k as createVirtualMedia, w as VirtualMediaDataSchema } from "./dist-Bbhn-cb5.mjs";
import "./src-goQ_UXNy.mjs";
import { a as defineMetadata, i as defineNode } from "./server-RmKl3RaO.mjs";
import { n as runCodeGenAgent, r as tool, t as createCodeGenAgent } from "./server-ZrJ53Klz.mjs";
...
//#region ../../nodes/node-html-video-gen/dist/metadata-0BNzwCRL.mjs
...
const HTMLVideoGenNodeConfigSchema = z$1.object({
	...
	width: z$1.number().int().min(320).max(4096).default(1280),
	height: z$1.number().int().min(320).max(4096).default(720),
	fps: z$1.number().min(12).max(60).default(24),
	durationSeconds: z$1.number().min(.5).max(30).default(5),
	format: z$1.enum(["mp4", "webm"]).default("mp4"),
	transparent: z$1.boolean().default(false)
}).strict().superRefine((data, ctx) => {
	if (data.transparent && data.format !== "webm") ctx.addIssue({
		code: z$1.ZodIssueCode.custom,
		path: ["transparent"],
		message: "Transparency is only supported with webm format."
	});
});
		...
		try {
			...
			const { agent, resultStore } = createCodeGenAgent({
				...
				async lint(result$1) {
					try {
						const problems = [];
						const { lintHyperframeHtml } = await import("./dist-DTPuP1jp.mjs");
			...
			logger.info(`Rendering HTML content to ${nodeConfig.format} video for node: ${node.id} via mediaRenderer service`);
			const renderResult = await this.mediaRenderer.renderHtmlVideo(generatedHtml, {
				userId: data.canvas.userId,
				format: nodeConfig.format,
				transparent: nodeConfig.transparent
			});
			let asset;
			if (renderResult.assetId) {
				...
				asset = await this.prisma.fileAsset.create({ data: {
					...
					mimeType: nodeConfig.format === "webm" ? "video/webm" : "video/mp4",
					isUploaded: true
				} });
			}
