import { M as generateId, O as createOutputItemSchema, _ as FileDataSchema, a as TOKENS, b as MultiOutputGenericSchema, c as logger, k as createVirtualMedia, s as getAssetKey, y as ModerationError } from "./dist-CZ1kEBl4.mjs";
...
//#region ../../nodes/node-image-gen/dist/metadata-C95vApb9.mjs
...
const OpenAIImageSizeSchema = z$1.union([z$1.enum(IMAGEGEN_OPENAI_PRESET_SIZES), z$1.object({
	width: z$1.number().multipleOf(16).min(256).max(3840),
	height: z$1.number().multipleOf(16).min(256).max(3840)
}).refine(({ width, height }) => {
...
const FalFluxImageSizeSchema = z$1.union([z$1.enum(IMAGEGEN_FAL_FLUX_PRESET_SIZES), z$1.object({
	width: z$1.number().multipleOf(16).min(256),
	height: z$1.number().multipleOf(16).min(256)
})]);
const SeedreamImageSizeSchema = z$1.union([z$1.enum(IMAGEGEN_SEEDREAM_PRESET_SIZES), z$1.object({
	width: z$1.number().multipleOf(16).min(256).max(2048),
	height: z$1.number().multipleOf(16).min(256).max(2048)
}).refine(({ width, height }) => {
	const pixels = width * height;
...
const QwenImageSizeSchema = z$1.union([z$1.enum(IMAGEGEN_QWEN_PRESET_SIZES), z$1.object({
	width: z$1.number().multipleOf(16).min(256).max(1440),
	height: z$1.number().multipleOf(16).min(256).max(1440)
}).refine(({ width, height }) => {
	const pixels = width * height;
...
const ImageGenNodeConfigSchema = z$1.object({
	...
	falFluxSize: FalFluxImageSizeSchema.catch(IMAGEGEN_FAL_FLUX_PRESET_SIZES[0]),
	falFluxSafetyTolerance: z$1.enum(IMAGEGEN_FAL_FLUX_SAFETY_TOLERANCES).catch("5"),
	falFluxEnableSafetyChecker: z$1.boolean().catch(false),
	falFluxOutputFormat: z$1.enum(IMAGEGEN_FAL_FLUX_OUTPUT_FORMATS).catch("jpeg"),
	openaiSize: OpenAIImageSizeSchema.catch("landscape_4_3"),
	openaiQuality: z$1.enum(IMAGEGEN_OPENAI_QUALITIES).catch("medium"),
	openaiFormat: z$1.enum(IMAGEGEN_OPENAI_FORMATS).catch("png"),
	openaiBackground: z$1.enum(IMAGEGEN_OPENAI_BACKGROUNDS).default("opaque"),
	seedreamSize: SeedreamImageSizeSchema.catch("auto_2K"),
	seedreamOutputFormat: z$1.enum(IMAGEGEN_SEEDREAM_OUTPUT_FORMATS).catch("jpeg"),
	nanoBananaAspectRatio: z$1.enum(IMAGEGEN_NANO_BANANA_ASPECT_RATIOS).catch("auto"),
	nanoBananaOutputFormat: z$1.enum(IMAGEGEN_NANO_BANANA_OUTPUT_FORMATS).catch("png"),
	nanoBananaResolution: z$1.enum(IMAGEGEN_NANO_BANANA_RESOLUTIONS).catch("1K"),
	nanoBananaEnableWebSearch: z$1.boolean().catch(false),
	nanoBananaThinkingLevel: z$1.enum(IMAGEGEN_NANO_BANANA_THINKING_LEVELS).catch("disabled"),
	qwenSize: QwenImageSizeSchema.catch("square_hd"),
	qwenOutputFormat: z$1.enum(IMAGEGEN_QWEN_OUTPUT_FORMATS).catch("png"),
	qwenEnablePromptExpansion: z$1.boolean().catch(true),
	qwenEnableSafetyChecker: z$1.boolean().catch(false)
});
