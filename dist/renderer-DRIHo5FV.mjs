...
import { a as computeLayerStylePadding } from "./config-Di1VDQUU-D0RfKgzn.mjs";

//#region ../../nodes/node-layer-style/dist/renderer.mjs
function hexToRgba(hex) {
	...
	if (cleaned.length === 3) cleaned = cleaned.split("").map((c) => c + c).join("") + "ff";
	else if (cleaned.length === 6) cleaned = cleaned + "ff";
	else if (cleaned.length === 8) {} else return [
		1,
		1,
		1,
		1
...
const BEVEL_STYLE_MAP = {
	InnerBevel: 0,
	OuterBevel: 1,
	Emboss: 2,
	PillowEmboss: 3
};
const BEVEL_TECHNIQUE_MAP = {
	Smooth: 0,
	ChiselHard: 1,
	ChiselSoft: 2
};
const STROKE_POS_MAP = {
	outside: 0,
	center: 1,
	inside: 2
};
...
`;
const resourcesCache = /* @__PURE__ */ new WeakMap();
function getLayerStyleResources(device, format) {
	let res = resourcesCache.get(device);
	if (res) return res;
	const module = device.createShaderModule({ code: WGSL_LAYER_STYLE_SHADER });
...
const WebGPURenderer = async (args) => {
	...
	const elapsedSeconds = props.elapsedMs !== void 0 ? props.elapsedMs / 1e3 : (props.frame ?? 0) / (props.fps || 30);
	const resolveField = (handleId, fallbackVal, clampMin = 0, clampMax = 1e3) => {
		if (handleId && op.inputs?.[handleId]?.connectionValid) {
			...
			if (!outputItem) return Math.max(clampMin, Math.min(clampMax, fallbackVal));
			...
			if (outputItem.type === "Signal") {
				...
				if (typeof data === "number") return Math.max(clampMin, Math.min(clampMax, data));
				if (data && typeof data === "object") {
					if (typeof data.value === "number") return Math.max(clampMin, Math.min(clampMax, data.value));
					...
					const offset = typeof data.offset === "number" ? data.offset : fallbackVal;
					const t = elapsedSeconds;
					const computed = offset + amp * Math.sin(2 * Math.PI * freq * t + phase);
	...
	const childTex = ctx.renderer.getTemporaryTexture(childW, childH, [...props.excludeTextures || [], targetTexture]);
	...
	const inputPass = ctx.renderer.beginFrame(encoder, inputView, {
		r: 0,
		g: 0,
		b: 0,
		a: 0
	}, width, height, "clear");
	ctx.renderer.drawTexture(inputPass, childTex, {
		x: padX,
		y: padY,
		width: childW,
		height: childH
	...
	inputPass.end();
	const { pipeline, uniformLayout, textureLayout } = getLayerStyleResources(ctx.device, ctx.renderer.format);
	...
	const dsOpacity = resolveField(op.dropShadowOpacityHandleId, ds.opacity ?? .75, 0, 1);
	const dsAngle = resolveField(op.dropShadowAngleHandleId, ds.angle ?? 120, 0, 360) * Math.PI / 180;
	const dsDist = resolveField(op.dropShadowDistanceHandleId, ds.distance ?? 10, 0, 500);
	const dsSpread = resolveField(op.dropShadowSpreadHandleId, ds.spread ?? 0, 0, 100);
	...
	const isOpacity = resolveField(op.innerShadowOpacityHandleId, is.opacity ?? .75, 0, 1);
	const isAngle = resolveField(op.innerShadowAngleHandleId, is.angle ?? 120, 0, 360) * Math.PI / 180;
	const isDist = resolveField(op.innerShadowDistanceHandleId, is.distance ?? 5, 0, 500);
	const isChoke = resolveField(op.innerShadowChokeHandleId, is.choke ?? 0, 0, 100);
	...
	const ogOpacity = resolveField(op.outerGlowOpacityHandleId, og.opacity ?? .75, 0, 1);
	const ogSize = resolveField(op.outerGlowSizeHandleId, og.size ?? 15, 0, 250);
	const ogSpread = resolveField(op.outerGlowSpreadHandleId, og.spread ?? 0, 0, 100);
	const ogBlend = BLEND_MODE_MAP[og.blendMode ?? "screen"] ?? 2;
	const igCol = hexToRgba(ig.color ?? "#ffffff");
	const igOpacity = resolveField(op.innerGlowOpacityHandleId, ig.opacity ?? .75, 0, 1);
	const igSize = resolveField(op.innerGlowSizeHandleId, ig.size ?? 15, 0, 250);
	const igSpread = resolveField(op.innerGlowSpreadHandleId, ig.spread ?? 0, 0, 100);
	...
	const stOpacity = resolveField(op.strokeOpacityHandleId, st.opacity ?? 1, 0, 1);
	...
	const beDepth = resolveField(op.bevelEmbossDepthHandleId, be.depth ?? 100, 1, 1e3);
	const beDir = be.direction === "Down" ? -1 : 1;
	const beSize = resolveField(op.bevelEmbossSizeHandleId, be.size ?? 5, 0, 250);
	const beSoften = resolveField(op.bevelEmbossSoftenHandleId, be.soften ?? 0, 0, 50);
	const beAngle = resolveField(op.bevelEmbossAngleHandleId, be.angle ?? 120, 0, 360) * Math.PI / 180;
	const beAltitude = resolveField(op.bevelEmbossAltitudeHandleId, be.altitude ?? 30, 0, 90) * Math.PI / 180;
	const beHlOpacity = resolveField(op.bevelEmbossHighlightOpacityHandleId, be.highlightOpacity ?? .75, 0, 1);
	const beShOpacity = resolveField(op.bevelEmbossShadowOpacityHandleId, be.shadowOpacity ?? .75, 0, 1);
	const coCol = hexToRgba(co.color ?? "#ff0000");
	const coOpacity = resolveField(op.colorOverlayOpacityHandleId, co.opacity ?? 1, 0, 1);
	...
	uniformBufferData[4] = ds.enabled ? 1 : 0;
	...
	uniformBufferData[16] = is.enabled ? 1 : 0;
	...
	uniformBufferData[28] = og.enabled ? 1 : 0;
	...
	uniformBufferData[40] = ig.enabled ? 1 : 0;
	...
	uniformBufferData[52] = st.enabled ? 1 : 0;
	...
	uniformBufferData[68] = be.enabled ? 1 : 0;
	...
	uniformBufferData[84] = co.enabled ? 1 : 0;
