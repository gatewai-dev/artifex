import { C as SingleOutputGenericSchema, E as appendOperation, N as getActiveMediaMetadata, O as createOutputItemSchema, a as TOKENS, b as MultiOutputGenericSchema, w as VirtualMediaDataSchema } from "./dist-CJI3Jl43.mjs";
import { a as defineMetadata, i as defineNode } from "./server-ClH_dFot.mjs";
import { z as z$1 } from "zod";
import { inject, injectable } from "inversify";

//#region ../../nodes/node-crop/dist/metadata-CePqJsGL.mjs
const CropNodeConfigSchema = z$1.object({
	cropType: z$1.enum(["rect", "path"]).default("rect").optional(),
	leftPercentage: z$1.number().min(0).max(100).default(0),
	topPercentage: z$1.number().min(0).max(100).default(0),
	widthPercentage: z$1.number().min(0).max(100).default(100),
	heightPercentage: z$1.number().min(0).max(100).default(100),
	pathPoints: z$1.array(z$1.object({
		x: z$1.number(),
		y: z$1.number()
	})).optional(),
	roundness: z$1.number().min(0).max(100).default(0).optional()
}).strict();
CropNodeConfigSchema.extend({
	op: z$1.literal("Crop"),
