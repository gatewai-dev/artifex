...
import { w as parseColor } from "./dist-xl6mY7se.mjs";
import { t as audioRegistry } from "./browser-G8bOolNE.mjs";
import { a as measureText } from "./dist-DQgw6Nnp.mjs";
import { t as defineRenderer } from "./renderer-BMji3WdP.mjs";
import { i as collectNodeOps, n as compileTimeline, o as computeLayout, s as initLayout, t as compileLayerTimeline } from "./compiler-Iia_EXww-DU73M2EN.mjs";
	...
	if (op.op === "Compositor") {
		...
		const renderLayerVM = async (nodeVM, i, destTex, destView, excludes, accumulatedOpacity, parentMatrix, parentRect) => {
			...
			if (kind === "media") content = nodeVM.children?.[0];
				...
				let displayText = lop.text ?? "";
				if (lop.animation?.tracks) {
					for (const track of lop.animation.tracks) for (const kf of track.keyframes) if (kf.presetType === "typewriter" || kf.presetType === "word-reveal" || kf.presetType === "karaoke") {
						const groupKfs = track.keyframes.filter((k) => k.presetGroupId === kf.presetGroupId);
						if (groupKfs.length >= 2) {
							const minFrame = Math.min(...groupKfs.map((k) => k.frame));
							const maxFrame = Math.max(...groupKfs.map((k) => k.frame));
							const relFrame = shiftedFrame - (lop.startFrame ?? 0);
							const dur = maxFrame - minFrame;
							if (dur > 0) {
								const progress = Math.max(0, Math.min(1, (relFrame - minFrame) / dur));
								if (kf.presetType === "typewriter") {
									const visibleChars = Math.floor(displayText.length * progress);
									displayText = displayText.substring(0, visibleChars);
								} else if (kf.presetType === "word-reveal") {
									const words = displayText.split(" ");
									const visibleWords = Math.floor(words.length * progress);
									displayText = words.slice(0, visibleWords).join(" ");
								}
							}
						}
						break;
