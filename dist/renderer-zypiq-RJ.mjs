...
//#region ../../nodes/node-selective-color/dist/renderer.mjs
...
`;
function getSelectiveColorResources(device, format) {
...
function packAdjustment(data, offset, adj) {
	data[offset + 0] = (adj?.cyan ?? 0) / 100;
	data[offset + 1] = (adj?.magenta ?? 0) / 100;
	data[offset + 2] = (adj?.yellow ?? 0) / 100;
	data[offset + 3] = (adj?.black ?? 0) / 100;
}
const uniformBufferData = new Float32Array(40);
const SelectiveColorWebGPURenderer = async (args) => {
	...
	uniformBufferData[36] = op.method === "Absolute" ? 1 : 0;
	uniformBufferData[37] = 0;
	uniformBufferData[38] = 0;
	uniformBufferData[39] = 0;
	const { pipeline, uniformLayout, textureLayout } = getSelectiveColorResources(ctx.device, ctx.renderer.format);
...
var renderers_default = defineRenderer({ WebGPURenderer: SelectiveColorWebGPURenderer });
