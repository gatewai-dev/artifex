import { C as SingleOutputGenericSchema, O as createOutputItemSchema, _ as FileDataSchema, w as VirtualMediaDataSchema } from "./dist-D86uNdKf.mjs";
import { z as z$1 } from "zod";

//#region ../../packages/node-sdk/dist/index.mjs
var ConfigBuilder = class ConfigBuilder$1 {
	constructor(shape, configHandles, fieldSchemas = {}) {
		this.shape = shape;
	...
	field(key, schema, options) {
		...
		return new ConfigBuilder$1(newShape, newHandles, newFieldSchemas);
	}
	build() {
		const schema = z$1.object(this.shape);
					...
					if (handleId) {
						...
						if (input && input.connectionValid && input.outputItem) {
							...
							if (signal && typeof signal === "object" && "type" in signal) {
								if (signal.type === "numeric") resolvedVal = signal.value;
								else if (signal.type === "matrix") resolvedVal = signal.matrix;
								else if (signal.type === "generator") {
									const t = frame / fps;
...
//#endregion
export { ImageResultSchema as a, TextResultSchema as c, DimensionSchema as i, VideoResultSchema as l, BaseLayerSchema as n, NumberResultSchema as o, ColorSchema as r, SignalResultSchema as s, AudioResultSchema as t, configBuilder as u };