import { E as appendOperation, N as getActiveMediaMetadata, a as TOKENS } from "./dist-CQCeP0r0.mjs";
import { a as defineMetadata, i as defineNode } from "./server-BW9d6tuA.mjs";
import { l as VideoResultSchema, t as AudioResultSchema } from "./dist-DgX-LFoS.mjs";
import { z as z$1 } from "zod";
import { inject, injectable } from "inversify";

//#region ../../nodes/node-audio-noise-gate/dist/metadata-Bqpmmsfu.mjs
const NoiseGateNodeConfigSchema = z$1.object({
	threshold: z$1.number().min(-120).max(0).default(-40).describe("dBFS level below which the gate closes"),
	attack: z$1.number().min(1e-4).max(1).default(.005).describe("Seconds to open (fade in) when signal exceeds threshold"),
	hold: z$1.number().min(.001).max(5).default(.05).describe("Seconds to stay open after signal drops below threshold"),
	release: z$1.number().min(.01).max(5).default(.1).describe("Seconds to close (fade out) after hold expires"),
	range: z$1.number().min(-120).max(0).default(-80).describe("dBFS floor when gate is closed (avoids harsh clicks/silence)")
});
