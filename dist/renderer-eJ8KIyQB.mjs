...
//#region ../../nodes/node-crop/dist/renderer.mjs
const maskWgsl = `
struct MaskUniforms {
	rect      : vec4<f32>,
	roundness : f32,
...
function getMaskPipeline(device, format) {
...
const CropWebGPURenderer = async (args) => {
	const { ctx, props, drawChild, targetTexture, targetView, targetWidth, targetHeight, encoder } = args;
	const { virtualMedia, containerWidth: propsContainerWidth, containerHeight: propsContainerHeight, opacity = 1 } = props;
	...
	let finalWp = Math.max(.01, Number(op.widthPercentage) || 100);
	let finalHp = Math.max(.01, Number(op.heightPercentage) || 100);
	if (isPath && pathPoints.length > 0) {
		const xs = pathPoints.map((p) => p.x);
		const ys = pathPoints.map((p) => p.y);
		finalLp = Math.min(...xs);
		finalTp = Math.min(...ys);
		finalWp = Math.max(.01, Math.max(...xs) - finalLp);
		finalHp = Math.max(.01, Math.max(...ys) - finalTp);
	}
	...
	if (!(roundness > 0 || isPath || isRest) && !isRest) {
		...
		ctx.renderer.drawTextureRegion(outPass, childTex, srcRect, dstRect, { opacity });
		args.pass = outPass;
		return;
	}
	let maskedSourceTex = childTex;
	if (!isRest) {
		const upscaledTex = ctx.renderer.getTemporaryTexture(containerWidth, containerHeight, [
			childTex,
			targetTexture,
			...props.excludeTextures || []
		]);
		const upscaledView = upscaledTex.createView();
		const upscaledPass = ctx.renderer.beginFrame(encoder, upscaledView, {
			r: 0,
			g: 0,
			b: 0,
			a: 0
		}, containerWidth, containerHeight, "clear");
		ctx.renderer.drawTextureRegion(upscaledPass, childTex, srcRect, dstRect);
		upscaledPass.end();
		maskedSourceTex = upscaledTex;
	} else if (sourceWidth !== containerWidth || sourceHeight !== containerHeight) {
		const upscaledTex = ctx.renderer.getTemporaryTexture(containerWidth, containerHeight, [
			childTex,
			targetTexture,
			...props.excludeTextures || []
	...
	const maskTex = ctx.renderer.getTemporaryTexture(containerWidth, containerHeight, [
		maskedSourceTex,
		targetTexture,
		...props.excludeTextures || []
	]);
	...
	const maskData = new Float32Array(136);
	...
	if (isPath) for (let i = 0; i < Math.min(pathPoints.length, 64); i++) {
	...
	maskPass.draw(4);
	maskPass.end();
	const resultTex = ctx.renderer.composite(encoder, maskedSourceTex, maskTex, isRest ? "mask-out" : "mask-in", [
		maskedSourceTex,
		maskTex,
		targetTexture,
		...props.excludeTextures || []
	...
	ctx.renderer.drawTexture(finalPass, resultTex, {
		x: 0,
		y: 0,
		width: containerWidth,
		height: containerHeight
...
var renderers_default = defineRenderer({ WebGPURenderer: CropWebGPURenderer });
