...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
    ...
    if (isnan(state[0]) || isinf(state[0])) { gateState = 0u; }

    var currentGain = state[1];
    if (isnan(currentGain) || isinf(currentGain)) { currentGain = 0.0f; }

    var gainAtTransition = state[2];
    if (isnan(gainAtTransition) || isinf(gainAtTransition)) { gainAtTransition = 0.0f; }
    ...
    if (isnan(sumSquares) || isinf(sumSquares)) { sumSquares = 0.0f; }
    ...
    for (var index = 0u; index < numSamples; index = index + 1u) {
        ...
        for (var c = 0u; c < numChannels; c = c + 1u) {
            ...
            if (isnan(inVal) || isinf(inVal)) { inVal = 0.0f; }
