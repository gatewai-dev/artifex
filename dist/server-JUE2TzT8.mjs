...
//#region ../../nodes/node-curves/dist/metadata-DoQA1Lm4.mjs
const CurvePointSchema = z$1.object({
	x: z$1.number().min(0).max(1),
	y: z$1.number().min(0).max(1)
});
const ScalePointSchema = z$1.object({
	x: z$1.number().min(0).max(1),
	y: z$1.number().min(0).max(2)
});
...
const CurvesNodeConfigSchema = z$1.object({
	...
	hueVsSat: z$1.array(ScalePointSchema).min(2).default([{
		x: 0,
		y: 1
	}, {
		x: 1,
	...
	lumVsSat: z$1.array(ScalePointSchema).min(2).default([{
		x: 0,
		y: 1
	}, {
		x: 1,
	...
	satVsSat: z$1.array(ScalePointSchema).min(2).default([{
		x: 0,
		y: 1
	}, {
		x: 1,
