...
import {
	UNSHARP_MASK_OUTPUT_TYPE_MAP,
	UnsharpMaskNodeConfigSchema,
	type UnsharpMaskResult,
} from "../shared/index.js";

@injectable()
export class UnsharpMaskProcessor implements NodeProcessor {
	constructor(
	...
	async process({
		...
		BackendNodeProcessorResult<UnsharpMaskResult>
	> {
		try {
			...
			} as unknown as UnsharpMaskResult;
			...
			return {
				success: false,
				error:
					err instanceof Error ? err.message : "UnsharpMask processing failed",
			};
		}
	}
}
